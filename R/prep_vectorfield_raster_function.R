...
js_dir <- "/Volumes/classes/GEOG572/Students/leatherl/sifviz/assets/vector_field/"
...
july_jan_mag_pts <- data.frame(rasterToPoints(july_jan_mag))

colnames(july_jan_mag_pts) <- c("lon", "lat", "sif")

#export as .csv
write.csv(july_jan_mag_pts, paste0(js_dir, "json/july_jan_mag_simple.csv"), row.names = FALSE)

# export as geojson
geojson_write(july_jan_mag_pts, file = paste0(js_dir, "json/july_jan_mag_simple.geojson"))

#export as to geojson for visualization in dc / crossfilter
jan_july_mag_pts <- data.frame(rasterToPoints(jan_july_mag))

colnames(jan_july_mag_pts) <- c("lon", "lat", "sif")

#export as .csv
write.csv(jan_july_mag_pts,paste0(js_dir, "json/jan_july_mag_simple.csv"), row.names = FALSE)
