...
d3.csv('assets/vector_field/json/july_jan_mag_simple.csv', function(july_mag) { // d3 v4 version
    d3.csv('assets/vector_field/json/jan_july_mag_simple.csv', function(jan_mag) { // d3 v4 version
    ...
    // if i also load in the jan_july mag... and parse the data differently... i can show the seasonal patterns for both subsets / datasets.
        ...
        var filter_n = crossfilter(july_mag); //passes the features to the crossfilter
        ...
        var sifDimension_n = filter_n.dimension(function(d1) {
            ...
            var sif = d1.sif; //sif is called "sif" in this file...
            return sif < -1 ? '< -1.0' :
                sif < -0.5 ? '-1.0 - -0.5' :
                    sif < 0 ? '-0.5 - 0' :
                        sif < 0.5 ? '0 - 0.5' :
        ...
        var geomDimension_n = filter_n.dimension(function(d1) {
            ...
            return geometry
             ...
             //"geometry": { "type": "Point", "coordinates": [ -52.75, 71.25 ] }
        });
        ...
        //create crossfilter passes each feature to the filter. We have to pass it through the crossfilter before we can split up with data using
        //the .dimensions() method
        var filter_s = crossfilter(jan_mag); //passes the features to the crossfilter
        ...
        var sifDimension_s = filter_s.dimension(function(d) {
            ...
            //lamda operations are simple version of if/else statements. EX: If mag is less than 1.2 return the first element, else continue to the second element
            var sif = d.sif; //sif is called "NA" in this file...
            return sif < -1 ? '< -1.0' :
                sif < -0.5 ? '-1.0 - -0.5' :
                    sif < 0 ? '-0.5 - 0' :
                        sif < 0.5 ? '0 - 0.5' :
        ...
        //returns the geometry for each feature; this is the points at which sif was observed
        var geomDimension_n = filter_s.dimension(function(d) {
            var lon = d.lon;
            var lat = d.lat;
            var geometry = { "type": "Point", "coordinates": [ lon, lat ] }
            return geometry
        ...
        sifChart_s
            ...
            .elasticY(true) //allows for an OTF adjustable y axis
            .x(d3.scaleBand()) //Do this because there are ORDERs of MAGNITUDE; Its the scale that we want in the chart
            .xUnits(dc.units.ordinal)
            .yAxis() //pulls in data from the dataset
            .ticks(3); //number of ticks in a chart

        ////// demo to hold space for timeseries graph
        var sifChart_3 = dc.barChart('#sif-chart-3');
        //define the characteristics of the chart before we actually add the data to it
        sifChart_3
            .height(100) //pixel height of the chart
            .margins({
